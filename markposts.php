...
if ($returnpage == 'index.php') {
    ...
    $returnto = partforum_go_back_to($returnpage.'?f='.$forum->id);
}
...
if ($mark == 'read') {
    if (!empty($d)) {
        if (! $discussion = $DB->get_record('partforum_discussions', array('id'=> $d, 'forum'=> $forum->id))) {
        ...
        if (partforum_tp_mark_forum_read($user, $forum->id,$currentgroup)) {
            add_to_log($course->id, "partforum", "mark read", "view.php?f=$forum->id", $forum->id, $cm->id);
        }
    }
...
//            add_to_log($course->id, "partforum", "mark unread", "view.php?f=$forum->id", $forum->id, $cm->id);
