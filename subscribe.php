...
if ($groupmode && !partforum_is_subscribed($user->id, $forum) && !has_capability('moodle/site:accessallgroups', $context)) {
...
if (partforum_is_subscribed($user->id, $forum->id)) {
    if (is_null($sesskey)) {    // we came here via link in email
        ...
        $PAGE->set_heading($course->fullname);
        echo $OUTPUT->header();
        echo $OUTPUT->confirm(get_string('confirmunsubscribe', 'partforum', format_string($forum->name)),
                new moodle_url($PAGE->url, array('sesskey' => sesskey())), new moodle_url('/mod/partforum/view.php', array('f' => $id)));
        echo $OUTPUT->footer();
        exit;
    }
    require_sesskey();
    if (partforum_unsubscribe($user->id, $forum->id)) {
        add_to_log($course->id, "partforum", "unsubscribe", "view.php?f=$forum->id", $forum->id, $cm->id);
    ...
    if (is_null($sesskey)) {    // we came here via link in email
        ...
        $PAGE->set_heading($course->fullname);
        echo $OUTPUT->header();
        echo $OUTPUT->confirm(get_string('confirmsubscribe', 'partforum', format_string($forum->name)),
                new moodle_url($PAGE->url, array('sesskey' => sesskey())), new moodle_url('/mod/partforum/view.php', array('f' => $id)));
        echo $OUTPUT->footer();
        exit;
    }
    ...
    add_to_log($course->id, "partforum", "subscribe", "view.php?f=$forum->id", $forum->id, $cm->id);
